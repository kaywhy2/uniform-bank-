...
export default function Hero() {
  ...
  return (
    <div style={{ margin: 0, padding: 0, display: "flex", flexDirection: "column", fontFamily: "Georgia, serif" }}>
      ...
      {/* ── Footer ── */}
      <footer style={{ width: "100%", background: "#fff", boxSizing: "border-box", borderTop: "1px solid #e5e5e5", padding: "clamp(32px, 5vw, 52px) clamp(16px, 5vw, 60px) clamp(24px, 4vw, 40px)" }}>
        <div className="hero-footer-grid">
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <span style={{ fontSize: 15, fontWeight: 700, color: "#7a6420", fontFamily: "Georgia, serif", letterSpacing: "0.02em" }}>Uniform Bank</span>
            <p style={{ margin: 0, fontSize: 13, color: "#666", lineHeight: 1.7, fontFamily: "Georgia, serif" }}>17 Holloway Lane, Suite 203, London — Bespoke tailoring and textile consultancy.</p>
          </div>

          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "#111", marginBottom: 4 }}>Explore</span>
            {["Collections", "Craft", "Services", "Clients"].map((link) => (
              <a key={link} href="#" style={{ fontSize: 13, color: "#555", textDecoration: "none", fontFamily: "Georgia, serif", transition: "color 0.2s" }}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "#111", marginBottom: 4 }}>Connect</span>
            <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
              {[
                ...
                { label: "X", href: "#", icon: <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.742l7.737-8.857L1.254 2.25H8.08l4.259 5.629 5.905-5.629zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg> },
                { label: "Facebook", href: "#", icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg> },
              ].map((s) => (
                <a key={s.label} href={s.href} style={{ width: 34, height: 34, borderRadius: "50%", border: "1px solid #ddd", display: "flex", alignItems: "center", justifyContent: "center", color: "#555", textDecoration: "none", transition: "border-color 0.2s, color 0.2s" }}
        ...
        <div style={{ borderTop: "1px solid #e5e5e5", marginTop: 32, paddingTop: 18, display: "flex", flexWrap: "wrap", justifyContent: "space-between", alignItems: "center", gap: 10 }}>
          <p style={{ margin: 0, fontSize: 12, color: "#aaa", fontFamily: "Georgia, serif" }}>© 2026 Uniform Bank. All rights reserved.</p>
          <div style={{ display: "flex", gap: 20 }}>
            {["Privacy Policy", "Terms of Use"].map((item) => (
              <a key={item} href="#" style={{ fontSize: 12, color: "#999", textDecoration: "none", fontFamily: "Georgia, serif", transition: "color 0.2s" }}
                onMouseEnter={(e) => { e.currentTarget.style.color = "#7a6420"; }}
                onMouseLeave={(e) => { e.currentTarget.style.color = "#999"; }}>{item}</a>
            ))}
          </div>
        </div>
      </footer>
