  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <style>{`
        ...
        .m-row { display: flex; justify-content: space-between; align-items: center; padding: 11px 0; border-bottom: 1px solid var(--border-light); gap: 16px; }
        .m-row:last-child { border-bottom: none; }
        .m-edit-row { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; padding: 6px 0; border-bottom: 1px solid var(--border-light); align-items: center; }
        .m-edit-row:last-child { border-bottom: none; }
        @media (max-width: 480px) { .m-edit-row { grid-template-columns: 1fr; gap: 4px; } }
      `}</style>

      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <p style={{ margin: 0, fontSize: 17, fontWeight: 500, color: "var(--text-main)" }}>My Measurements</p>
        {!isEditing && (
          <button
            onClick={handleEdit}
            style={{
              background: "transparent",
              border: `1px solid ${hasAny ? GOLD : "var(--border-strong)"}`,
              color: hasAny ? GOLD : "var(--text-muted)",
      ...
      <div style={{ background: "var(--bg-card)", border: "1px solid var(--border-light)", borderRadius: 12, padding: "6px 20px 12px" }}>
        ...
        {isEditing ? (
          <div style={{ marginTop: 10 }}>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
              <button onClick={handleSave} style={{ flex: 1, padding: "9px 0", fontSize: 13, fontWeight: 600, background: GOLD, color: "var(--bg-card)", border: "none", borderRadius: 8, cursor: "pointer", fontFamily: "inherit" }}>
                Save Measurements
              </button>
              <button onClick={handleCancel} style={{ flex: 1, padding: "9px 0", fontSize: 13, fontWeight: 500, background: "transparent", color: "var(--text-muted)", border: "1px solid var(--border-strong)", borderRadius: 8, cursor: "pointer", fontFamily: "inherit" }}>
                Cancel
              </button>
            </div>
          </div>
          ...
          <div style={{ marginTop: 8 }}>
            {MEASUREMENT_FIELDS.map(({ key, label }) => (
              <div key={key} className="m-row">
                ...
                <span style={{ fontSize: 13, fontWeight: 500, color: measurements[key] ? "var(--text-main)" : "var(--border-strong)" }}>
          ...
          <div style={{ textAlign: "center", padding: "28px 0 16px" }}>
            <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="var(--border-strong)" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" style={{ marginBottom: 10 }}>
            ...
            <p style={{ fontSize: 11, color: "var(--border-strong)", margin: "4px 0 0" }}>Click "Add Measurements" to get started.</p>
          </div>
        )}
      </div>
    </div>
...
function OrderRow({ item, onRemove, status }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "13px 0", borderBottom: "1px solid var(--border-light)" }}>
      <div style={{ width: 46, height: 46, borderRadius: 8, background: "var(--border-light)", flexShrink: 0, overflow: "hidden" }}>
        {image && <img src={image} alt={name} style={{ width: "100%", height: "100%", objectFit: "cover" }} />}
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <p style={{ margin: 0, fontSize: 13, fontWeight: 500, color: "var(--text-main)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{name}</p>
        <p style={{ margin: "2px 0 0", fontSize: 11, color: "var(--text-muted)" }}>{category}</p>
        <p style={{ margin: "1px 0 0", fontSize: 11, color: "#bbb" }}>Qty: {qty} · {formatNaira(unitPrice)}/unit</p>
      </div>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6, flexShrink: 0 }}>
        <p style={{ margin: 0, fontSize: 13, fontWeight: 500, color: "var(--text-main)" }}>{formatNaira(price)}</p>
        <Badge status={status} />
        <button
...
export default function Profile() {
  ...
  const inputStyle = {
    ...
    border: "1.5px solid rgba(0,0,0,0.12)", borderRadius: 8, color: "var(--text-main)",
    background: "var(--bg-card)", outline: "none", fontFamily: "Inter, sans-serif", transition: "border-color 0.18s",
  };
  ...
  const InfoRow = ({ label, value }) => (
    <div style={{ padding: "12px 0", borderBottom: "1px solid var(--border-light)", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
      <span style={{ fontSize: 12, color: "var(--text-muted)", minWidth: 80 }}>{label}</span>
      <span style={{ fontSize: 13, color: value ? "var(--text-main)" : "var(--border-strong)", fontWeight: value ? 500 : 400, textAlign: "right" }}>
        {value || "Not set"}
      </span>
    </div>
  );
    ...
    <div style={{ minHeight: "100vh", background: BG, paddingTop: 60, fontFamily: "Inter, sans-serif", display: "flex", flexDirection: "column" }}>
      <style>{`
        * { box-sizing: border-box; }
        ...
        .edit-input { width: 100%; padding: 8px 10px; font-size: 13px; border: 1.5px solid rgba(0,0,0,0.12); border-radius: 8px; color: var(--text-main); background: var(--bg-card); outline: none; transition: border-color 0.18s; font-family: inherit; box-sizing: border-box; }
        .edit-input:focus { border-color: ${GOLD}; }
        .pi-input:focus { border-color: ${GOLD}; }
        .pi-select:focus { border-color: ${GOLD}; }
        .nav-item { padding: 11px 16px; font-size: 13px; cursor: pointer; transition: all 0.15s; border-bottom: 1px solid var(--border-light); display: flex; align-items: center; justify-content: space-between; }
      ...
      <div className="p-wrap">
        <div className="p-grid">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div style={{ background: "var(--bg-card)", border: "1px solid var(--border-light)", borderRadius: 12, padding: "24px 18px 20px", textAlign: "center" }}>
              <div
              ...
              >
                {avatarSrc ? (
                  ...
                  <div style={{ width: 68, height: 68, borderRadius: "50%", background: GOLD, color: "var(--bg-card)", fontSize: 24, fontWeight: 500, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    {initials}
                  </div>
                )}
                <div style={{ position: "absolute", inset: 0, borderRadius: "50%", background: "var(--text-muted)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 3, opacity: avatarHovered ? 1 : 0, transition: "opacity 0.18s", pointerEvents: "none" }}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--bg-card)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"/>
                    <circle cx="12" cy="13" r="4"/>
                  </svg>
                  <span style={{ fontSize: 9, color: "var(--bg-card)", fontWeight: 500, letterSpacing: "0.05em" }}>CHANGE</span>
                </div>
              </div>
              ...
              {isEditing ? (
                <div style={{ textAlign: "left" }}>
                  <label style={{ fontSize: 10, color: "var(--text-muted)", fontWeight: 600, letterSpacing: "0.07em", textTransform: "uppercase", display: "block", marginBottom: 4 }}>Name</label>
                  <input className="edit-input" value={nameTemp} onChange={(e) => setNameTemp(e.target.value)} style={{ marginBottom: 10 }} />
                  <label style={{ fontSize: 10, color: "var(--text-muted)", fontWeight: 600, letterSpacing: "0.07em", textTransform: "uppercase", display: "block", marginBottom: 4 }}>Email</label>
                  <input className="edit-input" type="email" value={emailTemp} onChange={(e) => setEmailTemp(e.target.value)} style={{ marginBottom: 10 }} />
                  <label style={{ fontSize: 10, color: "var(--text-muted)", fontWeight: 600, letterSpacing: "0.07em", textTransform: "uppercase", display: "block", marginBottom: 4 }}>Phone</label>
                  <div style={{ position: "relative", marginBottom: 10 }}>
                    <div style={{ position: "absolute", left: 8, top: "50%", transform: "translateY(-50%)", display: "flex", alignItems: "center", gap: 4, fontSize: 12, color: "var(--text-muted)", pointerEvents: "none" }}>
                  ...
                  <label style={{ fontSize: 10, color: "var(--text-muted)", fontWeight: 600, letterSpacing: "0.07em", textTransform: "uppercase", display: "block", marginBottom: 4 }}>Sex</label>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    <button onClick={handleSave} style={{ flex: 1, padding: "8px 0", fontSize: 12, fontWeight: 600, background: GOLD, color: "var(--bg-card)", border: "none", borderRadius: 8, cursor: "pointer", fontFamily: "inherit" }}>Save</button>
                    <button onClick={handleCancel} style={{ flex: 1, padding: "8px 0", fontSize: 12, fontWeight: 500, background: "transparent", color: "var(--text-muted)", border: "1px solid var(--border-strong)", borderRadius: 8, cursor: "pointer", fontFamily: "inherit" }}>Cancel</button>
                  </div>
                </div>
              ) : (
                <>
                  <p style={{ margin: "0 0 3px", fontWeight: 600, fontSize: 15, color: "var(--text-main)" }}>{name}</p>
                  <p style={{ margin: "0 0 12px", fontSize: 12, color: "var(--text-muted)" }}>{email}</p>
                  {saved && <p style={{ margin: "0 0 8px", fontSize: 11, color: "#0F6E56", fontWeight: 500 }}>✓ Saved</p>}
                  <button onClick={handleEdit} style={{ background: "transparent", border: "1px solid var(--border-strong)", borderRadius: 8, padding: "6px 14px", fontSize: 11, color: "var(--text-muted)", cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 6, fontFamily: "inherit", marginBottom: 12 }}>
            ...
            <div onClick={() => navigate("/cart")} style={{ background: "var(--bg-card)", border: "1px solid var(--border-light)", borderRadius: 12, padding: "11px 16px", fontSize: 13, color: "#444", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between", transition: "background 0.15s" }}
              onMouseEnter={(e) => (e.currentTarget.style.background = GOLD_LIGHT)}
              onMouseLeave={(e) => (e.currentTarget.style.background = "var(--bg-card)")}
            >
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}><CartIcon /> My Cart</div>
              {totalOrders > 0 && (
                <span style={{ background: GOLD, color: "var(--bg-card)", fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 999 }}>{totalOrders}</span>
              )}
            </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            {activeNav === "Overview" && (
              <>
                <p style={{ margin: 0, fontSize: 17, fontWeight: 500, color: "var(--text-main)" }}>Account Overview</p>
                <div className="stat-grid">
                  {[
                    ...
                    <div key={label} style={{ background: "var(--bg-card)", border: "1px solid var(--border-light)", borderRadius: 10, padding: "16px 14px", textAlign: "center" }}>
                      <p style={{ margin: "0 0 4px", fontSize: 24, fontWeight: 600, color: "var(--text-main)", lineHeight: 1 }}>{value}</p>
                      <p style={{ margin: 0, fontSize: 10, color: "#bbb", textTransform: "uppercase", letterSpacing: "0.09em" }}>{label}</p>
                    </div>
                  ))}
                </div>

                <div style={{ background: "var(--bg-card)", border: "1px solid var(--border-light)", borderRadius: 12, padding: "18px 18px 4px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 2 }}>
                    <p style={{ margin: 0, fontSize: 14, fontWeight: 500, color: "var(--text-main)" }}>
                      Orders in Progress
                  ...
                  {inProgress.length === 0 ? (
                    <p style={{ fontSize: 12, color: "var(--border-strong)", padding: "18px 0 14px" }}>
                      No active orders.{" "}
                ...
                <div style={{ background: "var(--bg-card)", border: "1px solid var(--border-light)", borderRadius: 12, padding: "18px 18px 4px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 2 }}>
                    <p style={{ margin: 0, fontSize: 14, fontWeight: 500, color: "var(--text-main)" }}>
                      Recent Items
                      {recentItems.length > 0 && <span style={{ marginLeft: 8, fontSize: 11, background: "#f1f1f1", color: "var(--text-muted)", padding: "2px 8px", borderRadius: 999, fontWeight: 600 }}>{recentItems.length}</span>}
                    </p>
                    <span onClick={() => navigate("/cart")} style={{ fontSize: 12, color: GOLD, cursor: "pointer" }}>View all</span>
                  </div>
                  {recentItems.length === 0 ? (
                    <p style={{ fontSize: 12, color: "var(--border-strong)", padding: "18px 0 14px" }}>Nothing here yet.</p>
                  ) : (
            ...
            {activeNav === "Personal Info" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <p style={{ margin: 0, fontSize: 17, fontWeight: 500, color: "var(--text-main)" }}>Personal Info</p>
                  {!piEditing && (
                    <button onClick={handlePiEdit} style={{ background: "transparent", border: "1px solid var(--border-strong)", borderRadius: 8, padding: "6px 14px", fontSize: 11, color: "var(--text-muted)", cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 6, fontFamily: "inherit" }}>
                      <PencilIcon /> Edit
                    </button>
                  )}
                </div>
                <div style={{ background: "var(--bg-card)", border: "1px solid var(--border-light)", borderRadius: 12, padding: "20px 20px 8px" }}>
                  {piEditing ? (
                    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                      ...
                      <div>
                        ...
                        <div style={{ position: "relative" }}>
                          <div style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", display: "flex", alignItems: "center", gap: 5, fontSize: 13, color: "#555", pointerEvents: "none" }}>
                            🇳🇬 <span style={{ fontSize: 12, color: "var(--text-muted)" }}>+234</span>
                          </div>
                      ...
                      <div style={{ display: "flex", gap: 10, paddingTop: 4, paddingBottom: 8 }}>
                        <button onClick={handlePiSave} style={{ flex: 1, padding: "9px 0", fontSize: 13, fontWeight: 600, background: GOLD, color: "var(--bg-card)", border: "none", borderRadius: 8, cursor: "pointer", fontFamily: "inherit" }}>Save Changes</button>
                        <button onClick={handlePiCancel} style={{ flex: 1, padding: "9px 0", fontSize: 13, fontWeight: 500, background: "transparent", color: "var(--text-muted)", border: "1px solid var(--border-strong)", borderRadius: 8, cursor: "pointer", fontFamily: "inherit" }}>Cancel</button>
                      </div>
                    </div>
                  ) : (
